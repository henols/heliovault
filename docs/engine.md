...
- The blob is generated by `tools/levelc.py` and compiled into C via `src/levels/<level>.c`.
...
5) Player operates the door → transition action runs.

This keeps logic in data, not hardcoded.

---

## 8) Example: Minimal menu interaction loop (pseudocode)
