...
def compile_tset(ts: TileSet) -> tuple[bytes, str, dict, str, str, str]:
    ...
    blob_h = (
        "#pragma once\n"
        f"extern unsigned char {base}_tset_blob[];\n"
        f"extern unsigned long {base}_tset_blob_size;\n"
    )
    blob_c = (
        "// Auto-generated by tilesetc.py\n"
        f"#include \"tilesets/{base}_tset-blob.h\"\n"
        f"unsigned char {base}_tset_blob[] = {{\n"
        f"    #embed \"../../{GEN_ROOT}/assets/{ts.name}.bin\"\n"
        "};\n"
        f"unsigned long {base}_tset_blob_size = (unsigned long)sizeof({base}_tset_blob);\n"
    )
...
def main():
    ...
    if not args.ids:
        args.ids = os.path.join(GEN_ROOT, "include", "tilesets", f"{ts.name}_tset_ids.h")
    if not args.blob_h:
    ...
    if ts.charset_path:
        if not args.charset_h:
            args.charset_h = os.path.join(GEN_ROOT, "include", "charset", f"{ts.name}_charset-blob.h")
        if not args.charset_c:
            args.charset_c = os.path.join(GEN_ROOT, "src", "charset", f"{ts.name}_charset.c")
    if args.sym:
    ...
    if ts.charset_path and args.charset_h and args.charset_c:
        ...
        charset_h = (
            "#pragma once\n"
            f"extern unsigned char {base}_charset_blob[];\n"
            f"extern unsigned long {base}_charset_blob_size;\n"
        )
        charset_c = (
            "// Auto-generated by tilesetc.py\n"
            f"#include \"charset/{base}_charset-blob.h\"\n"
            f"unsigned char {base}_charset_blob[] = {{\n"
            f"    #embed \"{rel}\"\n"
            "};\n"
            f"unsigned long {base}_charset_blob_size = (unsigned long)sizeof({base}_charset_blob);\n"
        )
